...
export function ToolOutput({ toolName, command, output, error, success, timestamp }: ToolOutputProps) {
    ...
    return (
        <div className={`glass rounded-xl p-4 border ${success ? "border-green-500/20" : "border-red-500/20"}`}>
            {/* Header */}
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                    <span className="text-lg">{getToolIcon(toolName)}</span>
                    <span className="font-semibold text-sm">{toolName}</span>
                    <span className={`px-2 py-0.5 rounded-full text-xs ${success ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
