...
export const VoicePanel: React.FC = () => {
  ...
  const handlePttUp = useCallback(
    async (shouldCancelOverride?: boolean) => {
  ...
  const handlePttMove = useCallback(
    (clientX: number) => {
  ...
  const statusColor = () => {
    switch (status) {
      case "listening":
        return "bg-red-500/20 border-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.3)]";
      case "recording":
        return isCancelling
          ? "bg-zinc-500/20 border-zinc-500/50 grayscale"
          : "bg-orange-500/20 border-orange-500/50 shadow-[0_0_20px_rgba(249,115,22,0.3)]";
      case "processing":
        return "bg-blue-500/20 border-blue-500/50 shadow-[0_0_20px_rgba(59,130,246,0.3)]";
      default:
        return "bg-white/10 border-white/20 shadow-xl";
    }
  };
  ...
  const statusText = () => {
    switch (status) {
      case "listening":
        return "Listening...";
      case "recording":
        return isCancelling ? "Release to cancel" : "Recording...";
      case "processing":
        return "Transcribing...";
      default:
        return "Off";
      ...
      >
        ...
        >
          <div
            style={{
              transform:
                status === "recording" ? `translateX(${dragX}px)` : "none",
            }}
            className={`
                            p-3 rounded-xl transition-all duration-300
                            ${
        ...
        <div className="flex flex-col pr-2">
          ...
          {lastTranscription && status === "idle" && (
            <span
              className="text-xs text-white/40 truncate max-w-[120px]"
              title={lastTranscription}
            >
              {lastTranscription}
            </span>
          )}
