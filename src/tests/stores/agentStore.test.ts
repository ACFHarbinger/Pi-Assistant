...
describe('useAgentStore', () => {
    ...
    it('startAgent calls invoke and updates state', async () => {
        ...
        expect(mockInvoke).toHaveBeenCalledWith('start_agent', { task: 'test task', maxIterations: null });
        expect(useAgentStore.getState().messages).toHaveLength(1);
        expect(useAgentStore.getState().messages[0].content).toContain('Starting task: test task');
    });
    ...
    it('sendMessage adds message and invokes backend', async () => {
        ...
        expect(mockInvoke).toHaveBeenCalledWith('send_message', { message: 'hello' });
    });
});
