...
describe('Settings Component', () => {
    beforeEach(() => {
        ...
        vi.mocked(invoke).mockImplementation((cmd) => {
            ...
            switch (cmd) {
                case 'get_mcp_config': return Promise.resolve({ mcpServers: { git: { command: 'git', args: [] } } });
                case 'get_tools_config': return Promise.resolve({ enabled_tools: { shell: true } });
                case 'get_models_config': return Promise.resolve({ models: [] });
                case 'get_mcp_marketplace': return Promise.resolve([]);
    ...
    it('switches tabs', async () => {
        ...
        await waitFor(() => {
            ...
            expect(screen.getByText('Note: Only shows tools explicitly toggled in config.')).toBeInTheDocument();
        });
    });
