...
- Set up Cargo workspace with `pi-core` and `pi-protocol` crates
- Define JSON schemas in `protocol/schemas/`
- Implement `AppState`, `AgentState`, and basic Tauri commands
- Create minimal React frontend with status display
- Set up Python sidecar with `pyproject.toml`

### Phase 2: IPC Plumbing
- Implement NDJSON transport on both sides (Rust + Python)
- Build correlation ID routing and response matching
- Implement sidecar lifecycle management (start, health check, restart on crash)
- End-to-end test: Rust sends request -> Python processes -> Rust receives response

### Phase 3: Memory System
- SQLite schema creation and `rusqlite` integration
- Integrate `sqlite-vec` extension for vector search
- Implement embedding generation via Python sidecar
- Build the combined SQL + vector retriever
- Test: store messages, retrieve by semantic similarity

### Phase 4: Agent Loop
- Implement the core async agent loop with `CancellationToken`
- Build `ToolRegistry` and `Tool` trait
- Implement `ShellTool` and `CodeTool`
- Wire up human-in-the-loop (pause -> ask -> resume cycle)
- Test: agent executes multi-step tasks with tool calls

### Phase 5: Safety Layer
- Implement `PermissionEngine` with 3-tier regex rules
- Add path restrictions to `CodeTool`
- Add `PermissionDialog.tsx` to frontend
- Test: blocked commands fail, medium-risk prompts user, safe auto-approves

### Phase 6: Browser Tool
- Integrate `chromiumoxide` for headless Chrome
- Implement navigate/extract/screenshot actions
- Add network access controls (domain allowlist)

### Phase 7: Mobile Client
- Embed Axum WebSocket server in Tauri
- Implement pairing/authentication flow
- Build Android project: Gradle + Kotlin + Compose
- Implement `PiWebSocketClient`, `ConnectionManager`, `AgentViewModel`
- Voice input via `SpeechRecognizer`

### Phase 8: ML Training Pipeline
- Implement `InferenceEngine` with sentence-transformers + HF transformers
- Build `PiLightningModule` for fine-tuning
- Implement model registry with versioning
- Build PyInstaller bundling script for distribution

### Phase 9: Integration Testing & Polish
- End-to-end: mobile sends task -> desktop agent runs -> asks permission -> completes
- Error recovery: sidecar crash, network disconnect, timeout handling
- Finalize `.gitignore` for all build artifacts
